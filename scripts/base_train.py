...
if USE_FA3:
    ...
    if HAS_FA3 and COMPUTE_DTYPE != torch.bfloat16:
        print0(
            f"WARNING: Flash Attention 3 only supports bf16, but COMPUTE_DTYPE={COMPUTE_DTYPE}. "
            "Using PyTorch SDPA fallback"
        )
    else:
...
if args.fp8:
    if device_type != "cuda":
        ...
        print0(
            f"✓ FP8 training enabled ({args.fp8_recipe} scaling) - converted "
            f"{num_fp8}/{num_linear} linear layers, skipped {num_skipped} (too small)"
        )


def iter_fp8_modules(model):
    """Yield (parent module, attribute name, Float8Linear module) triples."""
...
def linear_from_fp8(fp8_module):
    """Build an nn.Linear-compatible view that shares the FP8 module parameters."""
    linear = Linear(
...
if weight_decay_scaled != args.weight_decay:
    print0(
        f"Scaling weight decay from {args.weight_decay:.6f} "
        f"to {weight_decay_scaled:.6f} for depth {args.depth}"
    )
